...
def process(
    ...
    # Calculate center points for each element
    parsed_content_with_centers = []
    for item in parsed_content_list:
        ...
        parsed_content_with_centers.append(item)

    # Format output string with center coordinates and bounding box
    parsed_content_str = "\n".join(
        [
            f'type: {x["type"]}, content: {x["content"]}, interactivity: {x["interactivity"]}, '
            + (
                f'center: ({x["center"][0]:.2f}, {x["center"][1]:.2f}), '
                f'bbox: ({x["bbox"][0]:.2f}, {x["bbox"][1]:.2f}, {x["bbox"][2]:.2f}, {x["bbox"][3]:.2f})'
                if "bbox" in x
                else "no coordinates"
            )
            for x in parsed_content_with_centers
        ]
    )
